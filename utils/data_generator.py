...
class DataGenerator():
    ...
    def generator(self, X=None, y=None, scale=True, normalize_cv_nlp=True,
                  la=None, at_least_one_labeled=False,
                  noise_type=None, duplicate_times:int=2, max_size=10000, skip_binary=True,
                  binary_to_logit=False, logit_eps=0.05):
        '''
        ...
        # disable scaling for CV/NLP datasets if requested
        if not normalize_cv_nlp and self.dataset in self.dataset_list_cv + self.dataset_list_nlp:
            scale = False

        # standard scaling
        if scale:
            if skip_binary or binary_to_logit:
                ...
                for col in df_train.columns:
                    uniques = set(df_train[col].dropna().unique())
                    if uniques.issubset({0.0, 1.0}):
                        binary_cols.append(col)
                    elif uniques.issubset({-1.0, 0.0, 1.0}):
                        trinary_cols.append(col)
                    else:
                        scale_cols.append(col)

                if binary_to_logit:
                    # Map binary {0,1} → logit, then StandardScale with continuous cols
                    for col in binary_cols:
                        for df in [df_train, df_test]:
                            x = np.clip(df[col].values.astype(float), logit_eps, 1 - logit_eps)
                            df[col] = np.log(x / (1 - x))
                    # Map trinary {-1,0,1} → [0,0.5,1] → logit
                    for col in trinary_cols:
                        for df in [df_train, df_test]:
                            x = np.clip((df[col].values.astype(float) + 1) / 2, logit_eps, 1 - logit_eps)
                            df[col] = np.log(x / (1 - x))
