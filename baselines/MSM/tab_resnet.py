...
class ResNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = nn.GroupNorm(num_groups=8, num_channels=bot_channels)
        ...
        self.norm2 = nn.GroupNorm(num_groups=8, num_channels=bot_channels)

        self.conv3 = nn.Conv2d(bot_channels, num_channels, kernel_size=1, stride=1)
        self.norm3 = nn.GroupNorm(num_groups=8, num_channels=num_channels)

        if use_1x1conv:
            self.conv4 = nn.Conv2d(
                num_channels, num_channels, kernel_size=1, stride=strides
            )
            self.norm4 = nn.GroupNorm(num_groups=8, num_channels=num_channels)
        else:
            self.conv4 = None

        self.film1 = FiLMBlock(time_emb_size, bot_channels, img_input=True)
        self.film2 = FiLMBlock(time_emb_size, bot_channels, img_input=True)
        self.film3 = FiLMBlock(time_emb_size, num_channels, img_input=True)
...
class TabResNet(nn.Module):
    ...
    def __init__(self, 
        ...
        """
        Tabular diffusion model with time conditioning.
        
        Args:
            ndims: Hidden dimension size
            act: Activation function ('relu', 'gelu', etc.)
            time_embedding_size: Dimension of time embeddings
            layers: Number of residual blocks
            dropout: Dropout probability
            numerical_features: Number of continuous features
            categories: List of categorical feature cardinalities
            embedding_type: 'fourier' or 'positional' time encoding
        """
        super().__init__()
        ...
        self.ndims = ndims
        self.act = act
        self.embedding_size = time_embedding_size
        self.layers = layers
        self.dropout = dropout
        self.input_dims = input_dims

        # Time embedding network
        TimeEncoder = (GaussianFourierProjection if embedding_type == "fourier" 
                    else PositionalEncoding)
        
        self.time_embed = nn.Sequential(
            TimeEncoder(embedding_size=time_embedding_size),
        ...
        self.resnet = nn.ModuleList([
            TabResBlockpp(ndims, ndims, time_embedding_size, act=act, dropout=dropout)
